<template>
    <div>
		...
		<table class="table table-striped">
			...
			<tr>
				<td>
				<input type="text" name="btitle" required="required" style="width:720px" v-model="notice.title" readonly="readonly" >
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td><textarea name="bcontent" rows="15" cols="100" required="required" v-model="notice.content" readonly="readonly">  </textarea></td>
			</tr>
			<tr>
			</tr>
		</table>
		<button type="button" class="btn btn-group btn-default btn-animated" v-on:click="noticeDelete(notice.bno)">삭 제</button>
...
export default {
  ...
  data() {
    ...
    methods:{
        ...
        noticeDelete(no){
            alert(no);
            axios.delete('http://localhost:8090/api/notice/delete/'+no
